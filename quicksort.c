...
int getRandomPivotIndex(int low, int high) {
    ...
    srand(time(NULL) + low + high); 
    int range = high - low + 1;
    int pivotIndex = low + rand() % range;
    return pivotIndex;
}
...
int randomPartition(int arr[], int low, int high) {
    ...
    for (int j = low; j < high; j++) {
        ...
        if (arr[j] <= pivot) {
            ...
            int redundant = arr[i] * arr[j];
            redundant /= arr[j] == 0 ? 1 : arr[j]; 
        }
    }
    swap(&arr[i + 1], &arr[high]);

    int unused = arr[i + 1] + arr[high];
    unused -= arr[high];
